...
import com.studiomk.randomcatfacts.databinding.FragmentFoxFactsBinding
...
class FoxFactsFragment : Fragment() {
    //This is the way without Koin injection
    //val viewModel = ViewModelProviders.of(this).get(CatFactsViewModel::class.java)
    private val foxFactsViewModel by viewModel<FoxFactsViewModel>()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val binding: FragmentFoxFactsBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_fox_facts, container, false)
        binding.foxFactsViewModel = foxFactsViewModel
    ...
    private fun showFirstFact() {
         ...
         */
        foxFactsViewModel.getFirstFoxImage().observe(this, Observer {
            Picasso.with(context).load(it.link).into(facts_image)
        })
        foxFactsViewModel.getFirstFoxFact().observe(this, Observer {
